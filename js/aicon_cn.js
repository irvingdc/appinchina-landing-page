...
function searchGoogle(){
	...
	googleRequest.onreadystatechange = function() {
	    if (this.readyState == 4 && this.status == 200) {
	    	...
	    	try{
	      		...
	      		googleResults += `<div onclick="handleSearchFocusOut()" class="aicon-clear flex"><span>清除搜索结果</span></div>`;
	...
	if(app != "" && app != lastAiconSearch){
		setTimeout(()=>{
			showLoader()
			find("#googleResults").innerHTML = `<div class="aicon-loading"><span>搜索中，请稍等…</span></div>`
			lastAiconSearch = app
...
function createStoresTable(){
	var html = `<div>
					<h2><span>你的应用</span> 可能会在以下的商店</h2>
					<p>预计下载次数：<span>0</span></p>
				</div>
				<table>
					<tr>
						<th>应用名称</th>
	    ...
	    if (this.readyState == 4 && this.status == 200) {
	    	...
	    	try{
	      		if(this.responseText && JSON.parse(this.responseText).text){
	      			...
	      			el.querySelector("td:nth-child(2)").innerHTML = `<a href="${response.href}" target="_blank">在商店中发现</a>`
		      		...
		      		el.querySelector("td:nth-child(3)").innerHTML = response.downloads ? response.styledDownloads : "查询到的结果"
	      		}
	      		else{
	      			el.querySelector("td:nth-child(2)").innerHTML = `没有查到`
	      		}
  		}
  		catch(err){
  			el.querySelector("td:nth-child(2)").innerHTML = `没有查到`
	    	...
	    	storesFound += 1
	    	el.querySelector("td:nth-child(2)").innerHTML = "商店暂无回应"
	    }
	}
	let timestamp = new Date().getTime()
