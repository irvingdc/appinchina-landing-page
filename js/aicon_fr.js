...
function searchGoogle(){
	...
	googleRequest.onreadystatechange = function() {
	    if (this.readyState == 4 && this.status == 200) {
	    	...
	    	try{
	      		...
	      		googleResults += `<div onclick="handleSearchFocusOut()" class="aicon-clear flex"><span>Effacer les résultats</span></div>`;
	...
	if(app != "" && app != lastAiconSearch){
		setTimeout(()=>{
			showLoader()
			find("#googleResults").innerHTML = `<div class="aicon-loading"><span>Patientez s'il vous plaît...</span></div>`
			lastAiconSearch = app
...
function createStoresTable(){
	var html = `<div>
					<h2><span>Votre application</span> peut être dans les stores suivants</h2>
					<p>Estimation du nombre de téléchargements: <span>0</span></p>
				</div>
				<table>
					<tr>
						<th>App Store</th>
	    ...
	    if (this.readyState == 4 && this.status == 200) {
	    	...
	    	try{
	      		if(this.responseText && JSON.parse(this.responseText).text){
	      			...
	      			el.querySelector("td:nth-child(2)").innerHTML = `<a href="${response.href}" target="_blank">Résultat trouvé</a>`
		      		...
		      		el.querySelector("td:nth-child(3)").innerHTML = response.downloads ? response.styledDownloads : "Trouvé dans les app stores"
	      		}
	      		else{
	      			el.querySelector("td:nth-child(2)").innerHTML = `Aucun résultat`
	      		}
  		}
  		catch(err){
  			el.querySelector("td:nth-child(2)").innerHTML = `Aucun résultat`
	    	...
	    	storesFound += 1
	    	el.querySelector("td:nth-child(2)").innerHTML = "Le Store ne répond pas"
	    }
	}
	let timestamp = new Date().getTime()
